    ...
    with Harness("aa-inbox-honesty") as harness:
        def mac_cli(*args: str, timeout: float = 60.0) -> str:
            ...
            result = subprocess.run(
                ...
                text=True,
                timeout=timeout,
                env={**os.environ, "HOME": str(harness.home), "TERM": "xterm-256color"},
            )
        ...
        # Walk to the droplet, whichever order the rail lists the two machines.
        for _ in range(4):
            if "droplet" in mac.snapshot():
                break
            mac.send(b"\x1b[B")
            time.sleep(0.6)
        ...
        # the whole value of the refusal is the sentence it carries.
        for _ in range(4):
            mac.send(b"\r")
            time.sleep(3.0)
            if "work allow" in mac.snapshot():
                break
            mac.send(b"m")
            time.sleep(0.5)
            mac.send(b"\x1b[B")
            time.sleep(0.5)
        refusal = mac.snapshot()
        check(
            "a refused remote terminal names the command that lifts it",
            "work allow" in refusal,
            "\n".join(line for line in refusal.split("\n") if "refused" in line)[:300],
        )
