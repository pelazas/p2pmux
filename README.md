...
`create` prints `Join with: p2pmux join <CODE>`, starts one fixed-size local shell and its host
view. `join` renders that remote pane. Short join codes resolve through a restrictive local cache
